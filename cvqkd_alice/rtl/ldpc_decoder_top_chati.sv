...
module ldpc_decoder_top #(
    ...
    // --- NUEVA: Interfaz de Carga del Síndrome de Bob ---
    input  logic                 syn_load_en,      // 1 = Escribiendo bloque de síndrome
    input  logic [5:0]           syn_load_addr,    // Fila del síndrome (0 a 45)
    input  logic [Z-1:0]         syn_load_data     // Bloque de 384 bits
);
    ...
    // Memoria interna (RAM estática) para almacenar el síndrome de la trama actual
    logic [Z-1:0] target_syndrome_mem [0:45];

    // Proceso de escritura síncrona (Hardware Real)
    always_ff @(posedge clk) begin
        if (syn_load_en) begin
            target_syndrome_mem[syn_load_addr] <= syn_load_data;
        end
    end
