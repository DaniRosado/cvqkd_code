...
module ldpc_decoder_top #(
    ...
    // --- Interfaz de Carga del Síndrome Objetivo (Bob) ---
    input  logic                 target_syn_we,
    input  logic [5:0]           target_syn_addr,
    input  logic [Z-1:0]         target_syn_data,

    // --- Interfaz de Lectura de la Clave Reconciliada (Extracción posterior) ---
    input  logic                 key_read_en,
    input  logic [6:0]           key_read_addr,
    output logic [Z-1:0]         key_read_data
);
    ...
    // Multiplexor de lectura para L_BRAM: Durante decodificación la lee la FSM,
    // al terminar la lee el extractor de clave del Wrapper
    logic [6:0] mux_p_read_addr;
    assign mux_p_read_addr = (key_read_en === 1'b1) ? key_read_addr : fsm_p_read_addr;
